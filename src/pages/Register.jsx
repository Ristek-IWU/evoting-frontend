...
function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    if (!name || !nim || !password) {
      const msg = "Nama, NIM, dan password wajib diisi";
      setError(msg);
    ...
    try {
      ...
      setToast({ show: true, type: "success", message: res.data.message || "Registrasi berhasil" });
      setName("");
      setNim("");
      setPassword("");
      ...
      <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
        ...
        >
          ...
          >
            ...
            <h2 className="text-2xl sm:text-3xl font-black text-center text-white mb-1 tracking-tight">
              Registrasi Akun
            </h2>

            <p className="text-center text-white/80 text-[10px] sm:text-sm mb-6 sm:mb-8">
              Daftarkan diri Anda untuk masa depan Himaif yang lebih baik
            </p>
            ...
            <form onSubmit={handleRegister} className="space-y-3 sm:space-y-4">
              ...
              {error && (
                <motion.p 
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="text-red-300 text-[11px] sm:text-xs text-center font-medium bg-red-500/10 py-2 rounded-lg"
                >
                  {error}
                </motion.p>
              )}
        ...
        >
          ...
          <h1 className="text-3xl lg:text-5xl font-black mb-4 leading-tight">
            Satu Suara <br/> <span className="bg-clip-text text-transparent bg-gradient-to-r from-pink-300 to-purple-200">Satu Masa Depan</span>
          </h1>

          <p className="max-w-xl text-base lg:text-lg text-white/80 leading-relaxed font-light">
            Setiap mahasiswa memiliki satu akun dan satu hak suara. 
            Pastikan data Anda benar untuk validasi pemilihan.
          </p>

          <p className="mt-6 italic text-white/60 text-sm lg:text-base">
            “Data aman, pemilihan transparan.”
          </p>
        </motion.div>
      </div>

      {/* ================= FOOTER (OPTIMIZED SINGLE SCREEN) ================= */}
      <footer className="px-4 sm:px-6 pb-6 sm:pb-8 shrink-0 relative z-20">
        <motion.div
          ...
          className="
            ...
            p-3 sm:p-5
            text-center text-white text-[10px] sm:text-sm
        ...
        >
          ...
          <span className="opacity-90 font-medium tracking-wide">© 2026 E-Voting Himaif — Registration Secured</span>
        </motion.div>
      </footer>
    </div>
  );
